...
table.insert(variables, { name = "re1", type = "any", ignoreunused = true, scope = 0, })
table.insert(variables, { name = "re2", type = "any", ignoreunused = true, scope = 0, })
table.insert(variables, { name = "re3", type = "any", ignoreunused = true, scope = 0, })
...
local function throw(_error, eargs, _type)
    ...
    if current_filename ~= "" then
        file_label = current_filename .. ":" .. line
    else
        file_label = "lcc"
    end
    if _type == "error" or _type == nil then
        print("\27[1;37m" .. file_label .. ": \27[1;31merror: \27[1;37m" .. (errors[_error] or "(no error text)") .. " " .. (eargs or "") .. "\27[0m") 
        if table.find(arg, "--error-extra-info") then 
            print(debug.traceback())
        end
        os.exit(tonumber(_error) or 1)
    elseif _type == "warning" then
        print("\27[1;37m" .. file_label .. ": \27[1;31mwarning: \27[1;37m" .. (errors[_error] or "(no warning text)") .. " " .. (eargs or "") .. "\27[0m")
    elseif _type == "info" then
        print("\27[1;37m" .. file_label .. ": \27[1;34minfo: \27[1;37m" .. (errors[_error] or "(no info text)") .. " " .. (eargs or "") .. "\27[0m") 
    end
end
    ...
    for i = 1, #tokens do
        ...
        if token.type == "identifier" and tokens[i + 1].value == "=" then
            ...
            if variable.type == "char_ptr" then 
                ...
                write(where, "mov r1, " .. var_name)
                for j = 1, string.len(value) - 2 do
                    write(where, "mov r2, \"" .. string.sub(value, j, j) .. "\"")
                    write(where, "str r1, r2")
                    write(where, "inc r1")
                end
                write(where, "mov r2, \"\\0\"")
                write(where, "str r1, r2")
                write(where, "inc r1")
            end
            next = _end + 1
        elseif token.type == "keyword" and vtype[token.value] then
            ...
            if tokens[i + 2].value == "=" then 
                ...
                if token.value == "int" then
                    ...
                    write("defs", var_name .. [[:
                        db ]] .. tohex16(tokens[i + 3].value)) 
                elseif token.value == "char*" then
                    local content = parseDef(vtokens) 
                    table.insert(variables, {
                        name = var_name,
                        type = "char_ptr", 
                        scope = scope,
        ...
        elseif token.type == "keyword" and not vtype[token.value] then 
            ...
            elseif token.value == "pragma" then
                ...
                --[[
                if tokens[i + 1].value == "__extern__" then
                    table.insert(variables, {
        ...
        elseif tokens[i].type == "identifier" and tokens[i + 1].value == "(" then
            ...
            if not table.find(reserved_varnames, tokens[i].value) then
                ...
                for j = 1, #args do
                    write(where, "push " .. args[j].value)
                end

                write(where, "jcl " .. tokens[i].value) 
            else 
...
while i <= #arg do
    ...
    if argument == "-o" then
        outfile = arg[i + 1]
        i = i + 2
    elseif argument == "--allow-implicit" or argument == "--error-extra-info" or argument == "-s" then
        i = i + 1
    else
...
if not table.find(arg, "-s") then
    ...
    if tonumber(returnvalue) ~= 0 then
        deleteTemp()
        throw(36, "error", "with exit code" .. returnvalue)
    end
    deleteTemp() 
else
