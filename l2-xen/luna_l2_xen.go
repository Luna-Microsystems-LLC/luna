...
func execute() {
	for {
		...
		if ProgramCounter == 0x0000 {
			setRegister(0x001a, uint32(0x0003))
			continue
		}
		if ProgramCounter == 0x0003 {
			codesect := uint32(Memory[ProgramCounter])<<8 | uint32(Memory[ProgramCounter+1])
		...
		case 0x0b:
			...
			var value uint32	
			if mode == 0x1 {
				value = uint32(Memory[ProgramCounter + 2]) << 24 | uint32(Memory[ProgramCounter + 3]) << 16 | uint32(Memory[ProgramCounter + 4]) << 8 | uint32(Memory[ProgramCounter + 5])
				setRegister(0x001a, ProgramCounter + 6)
				Log("push " + fmt.Sprintf("0x%08x", value))
			} else if mode == 0x2 {
				value = getRegister(uint32(Memory[ProgramCounter + 2]))
				setRegister(0x001a, ProgramCounter + 3)
				Log("push " + getRegisterName(uint32(Memory[ProgramCounter + 2])))
			}	
			sp := getRegister(0x0019)
			sp += 2
			Memory[video.Clamp(sp, 0, 0xffffffff - 1)] = byte(value & 0xFF)
			Memory[video.Clamp(sp + 1, 0, 0xffffffff - 1)] = byte(value >> 8)	
		...
		case 0x0c:
			...
			sp := getRegister(0x0019)
			value := uint32(Memory[sp]) | uint32(Memory[sp+1])<<8
		...
		case 0x18:
			...
			setRegister(toregister, uint32(Memory[video.Clamp(addr, 0, 0xffffffff - 1)]))
			setRegister(0x001a, ProgramCounter + 3)
		...
		case 0x19:
			...
			Memory[video.Clamp(addr, 0, 0xffffffff - 1)] = byte(getRegister(value) >> 24)
			Memory[video.Clamp(addr + 1, 0, 0xffffffff - 1)] = byte(getRegister(value) >> 16)
			Memory[video.Clamp(addr + 2, 0, 0xffffffff - 1)] = byte(getRegister(value) >> 8)
			Memory[video.Clamp(addr + 3, 0, 0xffffffff - 1)] = byte(getRegister(value) & 0xFF)
			setRegister(0x001a, ProgramCounter + 3)
			Log("str " + getRegisterName(uint32(Memory[ProgramCounter + 1])) + ", " + getRegisterName(value))
			stall(100)
		case 0x20:
			// LODDW
			// loddw <addr (register)> <destination register>
			addr := getRegister(uint32(Memory[ProgramCounter+1]))
			toregister := uint32(Memory[ProgramCounter+2])
			setRegister(toregister, uint32(Memory[video.Clamp(addr, 0, 0xffffffff - 1)]) << 24 | uint32(Memory[video.Clamp(addr + 1, 0, 0xffffffff - 1)])) << 16 | uint32(Memory[video.Clamp(addr + 2, 0, 0xffffffff - 1)])) << 8 | uint32(Memory[video.Clamp(addr + 3, 0, 0xffffffff - 1)]))
			setRegister(0x001a, ProgramCounter + 3)
			Log("loddw " + getRegisterName(uint32(Memory[ProgramCounter + 1])) + ", " + getRegisterName(toregister))
			stall(100)
		default:
...
func WindowManage(window *app.Window) error {
	...
	for {
		...
		case app.FrameEvent:	
			...
			scaleX := float32(GTX.Constraints.Max.X) / float32(640)
			scaleY := float32(GTX.Constraints.Max.Y) / float32(480)
...
func main() {
	...
	go func() {
		...
		setRegister(0x0019, uint32(len(data)))
		execute()
	}()	
	InitializeWindow()
}
