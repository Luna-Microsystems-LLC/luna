...
func IntHandler(code uint32) {
	if code == 0x01 {
		...
		video.MemoryVideo[video.Clamp(address, 0, 63999)] = byte(uint32(word) >> 8)
		video.MemoryVideo[video.Clamp(address + 1, 0, 63999)] = byte(uint32(word) & 0xFF)
	} else if code == 0x4 {
		// BIOS configure input mode
		// Mode 1: no type output
		// Mode 2: type output
		// In R1
		if getRegister(0x0001) == uint32(1) {
			TypeOut = true
		} else {
			TypeOut = false
		}
		...
		if KeyTrap == true {
			KeyTrap = false
			setRegister(uint32(0x0001), getRegister(0x001b))
		}
	} else if code == 0x6 {
		// BIOS wait for key
